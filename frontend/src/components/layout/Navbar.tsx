...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/60 bg-background/80 backdrop-blur-md">
            ...
            <>
              {isDashboard && isClientView && (
                <Link
                  ...
                  className="hidden sm:inline-flex items-center gap-1.5 rounded-xl bg-primary px-3.5 py-2 text-xs font-semibold text-primary-foreground shadow-sm hover:bg-primary/90 transition-all"
                >
                ...
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-60 rounded-2xl border border-border/80 bg-card p-2 shadow-xl z-50 animate-in fade-in zoom-in-95">
                    ...
                    {isDashboard && isFreelancerView && (
                      <>
                        <Link href="/freelancer/dashboard" className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground" onClick={() => setUserDropdownOpen(false)}>
                          <LayoutDashboard className="h-3.5 w-3.5" />
                          Overview
                        </Link>
      ...
      {/* Sub-Header Categories Bar (Fiverr Style) for Client */}
      {isDashboard && isClientView && (
        <div className="border-t border-border/40 bg-card/50 overflow-x-auto scrollbar-none hidden md:block">
          <Container className="flex items-center gap-6 h-10 text-xs font-medium text-muted-foreground whitespace-nowrap">
            <Link href="/client/talent?cat=Frontend" className="hover:text-primary transition-colors">Frontend & React</Link>
            <Link href="/client/talent?cat=Backend" className="hover:text-primary transition-colors">Backend & API</Link>
            <Link href="/client/talent?cat=Design" className="hover:text-primary transition-colors">UI/UX & Product Design</Link>
            <Link href="/client/talent?cat=AI" className="hover:text-primary transition-colors">AI Agents & Machine Learning</Link>
            <Link href="/client/talent?cat=Mobile" className="hover:text-primary transition-colors">Mobile Apps</Link>
            <Link href="/client/talent?cat=DevOps" className="hover:text-primary transition-colors">Cloud & DevOps</Link>
          </Container>
        </div>
      )}
